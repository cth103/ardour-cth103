...
if [ "$debug" == "1" ]; then
    gdb ./libs/canvas/benchmark/$name $3
elif [ "$callgrind" == "1" ]; then
    valgrind --tool="callgrind" ./libs/canvas/benchmark/$name $3
elif [ "$valgrind" == "1" ]; then
    valgrind --tool="memcheck" ./libs/canvas/benchmark/$name $3
else
    ./libs/canvas/benchmark/$name $2
fi
