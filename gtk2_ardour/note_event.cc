...
PBD::Signal1<void, NoteEvent*> NoteEvent::NoteEventDeleted;

/// dividing the hue circle in 16 parts, hand adjusted for equal look, courtesy Thorsten Wilms
const uint32_t NoteEvent::midi_channel_colors[16] = {
...
NoteEvent::NoteEvent(MidiRegionView& region, Item* item, const boost::shared_ptr<NoteType> note)
	: _region(region)
	, _item(item)
	, _text(0)
...
void
NoteEvent::on_channel_selection_change(uint16_t selection)
{
...
void
NoteEvent::on_channel_change(uint8_t channel)
{
...
void
NoteEvent::show_channel_selector(void)
{
	if (_channel_selector_widget == 0) {
		...
		_channel_selector->channel_selected.connect(
			sigc::mem_fun(this, &NoteEvent::on_channel_change));

		_channel_selector->clicked.connect (
			sigc::mem_fun (this, &NoteEvent::hide_channel_selector));
...
void
NoteEvent::hide_channel_selector(void)
{
...
void
NoteEvent::set_selected(bool selected)
{
	if (!_note) {
		return;
	}
...
NoteEvent::base_color()
{
	...
	case ChannelColors:
		return UINT_INTERPOLATE (UINT_RGBA_CHANGE_A (NoteEvent::midi_channel_colors[_note->channel()],
		                                             opacity), 
		                         ARDOUR_UI::config()->canvasvar_MidiNoteSelected.get(), 0.5);
...
void
NoteEvent::set_mouse_fractions (GdkEvent* ev)
{
	double ix, iy;
...
bool
NoteEvent::on_event(GdkEvent* ev)
{
...
bool
NoteEvent::mouse_near_ends () const
{
...
bool
NoteEvent::big_enough_to_trim () const
{
