	...
	: NoteBase (region, note)
{
	...
	if (with_events) {
		_rectangle->Event.connect (sigc::mem_fun (*this, &Note::event_handler));
	}
}
...
bool
Note::event_handler (GdkEvent* ev)
{
	if (!NoteBase::on_event (ev)) {
		return _region.get_time_axis_view().editor().canvas_note_event (ev, _rectangle);
	}
