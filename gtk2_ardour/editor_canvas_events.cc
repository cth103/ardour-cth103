...
bool
Editor::canvas_start_xfade_event (GdkEvent *event, Canvas::Item* item, AudioRegionView *rv)
{
	if (!rv->sensitive()) {
		return false;
	}
...
bool
Editor::canvas_end_xfade_event (GdkEvent *event, Canvas::Item* item, AudioRegionView *rv)
{
	if (!rv->sensitive()) {
		return false;
	}
...
bool
Editor::key_press_handler (Canvas::Item *, GdkEvent *, ItemType)
{
	return false;
}

bool
Editor::key_release_handler (Canvas::Item* item, GdkEvent* event, ItemType type)
{
	bool handled = false;
